...
def _apply_confidence_lift() -> None:
    ...
    try:
        ...
        if data.get("reset_gating"):
            ...
            prior = performance_oil.get_perf_dashboard_dict().get("confidence_score")
            performance_oil.reset_to_50(reason=reason)
            invalidate_cache()   # so the reset reflects on the next state push, not the next trade
