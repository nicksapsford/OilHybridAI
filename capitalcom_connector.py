...
log = logging.getLogger("OilHybrid.Excalibur")
...
class CapitalComConnector:
    ...
    def get_price(self, epic: str = FTSE_EPIC) -> Optional[dict]:
        ...
        try:
            ...
            # TODO BEFORE GO-LIVE: verify scalingFactor, onePipMeans and lot-size for
            # OIL_BRENT from /markets/{epic} (instrument block) and apply correct
            # scaling to price/size/P&L. Current paper trading assumes 1pt=$1/bbl and
            # uses raw bid/ask with no scaling factor applied. See get_market_details().
    ...
    def get_market_details(self, epic: str = FTSE_EPIC) -> Optional[dict]:
        """Return the FULL raw /markets/{epic} response (instrument metadata +
        dealingRules + snapshot). Used to inspect scalingFactor / onePipMeans /
        lotSize / minDealSize before go-live -- the values that define what one
        'point' actually is in USD. Returns None if not connected or on error."""
        self._refresh_session()
        if not self._connected:
            log.warning("get_market_details called but not connected to Capital.com")
            return None
        try:
            resp = requests.get(
                f"{self._base_url}/markets/{epic}",
            ...
            log.error("get_market_details failed for %s: %s", epic, exc)
            return None
