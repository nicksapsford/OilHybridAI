...
def build_system_brief(state, system_name, asset_label, logs_dir=None, now_utc=None):
    ...
    a("")
    a("EXTENDED RALLY FILTER")
    _so = state.get("session_open")
    _ext = state.get("extension")
    _lim = state.get("extended_limit")
    if _so is not None and _ext is not None and _lim is not None:
        _status = "BLOCKED" if abs(_ext) > _lim else "CLEAR"
        a("Session open: $%s | Extension: %+.2f | Limit: $%.2f | Status: %s"
          % (_num(_so, 2), _ext, _lim, _status))
    else:
        a("Session open not yet set (awaiting first tick after 22:00 UTC).")
    a("")
    a("STAY OUT QUALITY")
